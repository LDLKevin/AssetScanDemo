...
import com.eitc.assetscan.logic.ReticleGeometry;
...
public class ScannerOverlayView extends View {
    ...
    // 取景框垂直中心點（由上往下量）：上移至畫面偏上處，全盤／抽盤共用同一比例。
    private static final float CENTER_Y_FRACTION = 0.35f;
    ...
    /** 取景框位置變動時通知外部（例如掃描提示文字要貼著框下緣）。 */
    public interface OnFrameChangeListener {
        void onFrameChanged(RectF frame);
    }
    @Nullable private OnFrameChangeListener frameChangeListener;
    ...
    /** 取景框位置變動（含第一次量測完成）時的回呼；設定當下若已量測過會立即補發一次。 */
    public void setOnFrameChangeListener(@Nullable OnFrameChangeListener listener) {
        frameChangeListener = listener;
        if (listener != null && frame.width() > 0 && frame.height() > 0) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        buildBracketPath();
        if (frameChangeListener != null) frameChangeListener.onFrameChanged(frame);
    }
